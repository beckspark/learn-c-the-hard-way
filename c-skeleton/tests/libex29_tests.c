...
int check_function(const char *func_to_run, const char *data, int expected)
{
    ...
    check(func != NULL, "Did not find the %s function in the library %s: %s", func_to_run, lib_file, dlerror());
...
char *test_failures()
{
    mu_assert(check_function("fail_on_purpose", "Hello", 1), "fail_on_purpose should fail. On purpose.");
